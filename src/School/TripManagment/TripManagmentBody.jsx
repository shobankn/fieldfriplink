      ...
      try {
        ...
        const formattedTrips = res.data.trips.map(trip => ({
          ...
          date: trip.tripType === 'recurring'
            ? (trip.recurringDays?.length ? trip.recurringDays.map(day => dayMap[day.toLowerCase()] || day).join(', ') : 'N/A')
          ...
          driver: trip.assignedDrivers && trip.assignedDrivers.length > 0
            ? trip.assignedDrivers.map(driver => ({
                ...
                schoolRating: driver.mySchoolAverageRating ?? null
              }))
            : [],
        }));
