...
const InviteDriverPopup = ({ isOpen, onClose, tripId, onPublish }) => {
  ...
  useEffect(() => {
    ...
    const fetchDrivers = async () => {
      try {
        ...
        const res = await axios.get(`${baseURL}/school/drivers?status=approved`, {
    ...
    fetchDrivers();
        ...
        console.log('Selected Driver ID:', selectedDriver?._id);
  ...
  const handleSendJobPost = async () => {
    ...
    try {
      ...
      toast.warning('Driver already invited to this trip.', { autoClose: 3000 });
    } finally {
      setInviting(false);
    }
  };
...
//   };
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 bg-black/20 backdrop-blur-sm transition-opacity trans flex items-center justify-center z-50 p-4"
    >
      <motion.div
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row justify-end p-4 border-t border-gray-200 space-y-3 sm:space-y-0 sm:space-x-3">
          <button
            onClick={onClose}
            className="px-4 py-2 cursor-pointer  border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-100"
          >
            Cancel
          </button>
          <button
            onClick={handleSendJobPost}
            disabled={inviting || !selectedDriver}
            className={`px-4 py-2 cursor-pointer flex items-center justify-center rounded-lg text-white ${
              inviting || !selectedDriver ? 'bg-gray-400 cursor-not-allowed' : 'bg-red-500 hover:bg-red-600'
            }`}
          >
