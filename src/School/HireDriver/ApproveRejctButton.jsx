...
const DriverActions = ({ driverId }) => {
  ...
  return (
    <>
      <ToastContainer />
      <div className="flex gap-3">
        {/* Show Verify, Reject, and Suspend buttons if status is pending or rejected */}
        {['pending', 'rejected'].includes(driverStatus) && (
          <>
            {/* Verify button */}
            <motion.button
            ...
            {/* <motion.button
              ...
              whileHover={{ scale: loading ? 1 : 1.05 }}
              whileTap={{ scale: loading ? 1 : 0.95 }}
              className={`flex text-[14px] sm:text-[16px] px-3 py-2 sm:px-4 sm:py-2 text-white bg-yellow-500 rounded-[10px] inter-medium transition-colors ${
            ...
            >
              {loading ? (
                ...
                <>
                  <PauseCircle className="w-4 h-4 sm:w-5 sm:h-5 mr-1 my-auto" />
                  Suspend
                </>
              )}
            </motion.button> */}
        ...
        {/* Commented out: Suspend button for approved status */}
